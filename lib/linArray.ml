...
module Make(M:S.S) : S.LIN_ARRAY with module LinMonad = M = struct
  ...
  let alloc : 'a. 'a list -> (unit, 'a larr, unit data) monad =
    fun xs -> {__m=(fun  _ -> IO.return ({__lin=Array.of_list xs}, {data=()}))}

  let dealloc : 'a. ('a larr, unit, unit data) monad =
    {__m=(fun _ -> IO.return ((), {data=()}))}
  let to_list : 'a. ('a larr, 'a larr, 'a list data) monad =
    {__m=(fun larr -> IO.return (larr, {data=Array.to_list larr.__lin}))}
  let lookup : 'a.
               int -> ('a larr, 'a larr, 'a data) monad =
    fun i -> {__m=(fun larr -> IO.return (larr, {data=larr.__lin.(i)}))}

  let update : 'a.
               int -> 'a -> ('a larr, 'a larr, unit data) monad =
    fun i v -> {__m=(fun larr -> larr.__lin.(i) <- v; IO.return (larr, {data=()}))}

  let map : 'a 'b.
            ('a -> 'b) -> ('a larr, 'b larr, unit data) monad =
    fun f -> {__m=(fun larr -> IO.return ({__lin=Array.map f larr.__lin}, {data=()}))}
